...
如需接近支援結束的 Office 2010 伺服器的詳細資訊，請參閱[資源以協助您升級從 Office 2010 伺服器和用戶端](https://docs.microsoft.com/en-us/office365/enterprise/upgrade-from-office-2010-servers-and-products)。

## <a name="what-are-my-options"></a>我的選項為何？

即將達到其結尾支援 Exchange 2010，這是更好的時間來探索您的選項和準備的移轉規劃。您可以：

- 完全移轉至 Office 365。使用轉換、 最小混合或完整的混合式遷移遷移信箱然後移除內部部署 Exchange 伺服器] 和 [Active Directory]。
- 移轉至 Exchange 2016 的 Exchange 2010 伺服器與內部伺服器上。

> [!IMPORTANT]
> 如果您的組織選擇將信箱遷移至 Office 365，但要繼續進行管理使用者帳戶從內部部署 Active Directory 就地保留 DirSync 或 Azure AD 連線，您需要保留至少一個 Exchange server 的內部。如果移除前一個 Exchange server，您將不能夠變更 Exchange Online 中的 Exchange 收件者。這是因為授權來源會保留在內部部署 Active Directory 中且需要有進行變更。在此案例中，您有下列選項：

- （**建議使用**）如果您可以將您的信箱移轉至 Office 365 及 2020 年 1 月 14，以升級您的伺服器使用 Exchange 2010 來連線至 Office 365 和遷移的信箱。接著，將 Exchange 2010 移轉至 Exchange 2016 並解除委任任何剩餘的 Exchange 2010 伺服器。
- 如果您不能完成移轉信箱 2020 年 1 月 14、 由內部部署伺服器升級您的內部部署 Exchange 2010 伺服器以 Exchange 2016 先升級，然後使用 Exchange 2016 連線至 Office 365 和遷移的信箱。

> [!NOTE]
> 雖然這一點更很複雜，您可能也同時將內部部署 Exchange 2010 伺服器移轉至 Exchange 2016 將信箱遷移至 Office 365。

下列各節探索更詳細地每個選項。

## <a name="migrate-to-office-365"></a>移轉至 Office 365

將您的電子郵件移轉至 Office 365 是您最佳與最簡單的選項可幫助您淘汰 Exchange 2010 部署。使用 Office 365 遷移，可以進行單一一個躍點從舊的技術的藝術狀態功能類似：
...
| **遷移選項**     | **組織大小** | **期間**        |
|--------------------------|-----------------------|---------------------|
| 完全移轉        | 少於 150 基座  | 一週或更少      |
| 最少的混合式遷移 | 少於 150 基座  | 幾週或更少 |
| 完整的混合式遷移    | 多個 150 基座   | 幾週或更多 |
...
### <a name="minimal-hybrid-migration"></a>最少的混合式遷移

最少的混合式或 express、 移轉是其中包含您要移轉至 Office 365、 可以完成幾週內移轉並不需要任何進階的混合式遷移功能，例如共用空閒/忙碌行事曆的一些好幾百種信箱資訊。

最少的混合式遷移是更好的組織所需執行以將其信箱移轉至 Office 365 中，但仍計畫，以完成遷移幾週內的更多時間。您要取得沒有許多不同的複雜度更進階的完整的混合式移轉的一些優點。您可以控制多少，且哪一個、 信箱在指定的時間 ； 移轉Office 365 信箱將建立的使用者名稱與密碼的內部部署帳戶 ；與不同轉換遷移使用者不需要重新建立其 Outlook 設定檔。

如果您正在思考執行最少的混合式遷移，以下是要考慮的一些事項：
...
若要深入了解最少的混合式遷移，仔細[快速移轉 Exchange 信箱移轉至 Office 365 使用最少的混合式](https://support.office.com/article/Use-Minimal-Hybrid-to-quickly-migrate-Exchange-mailboxes-to-Office-365-fdecceed-0702-4af3-85be-f2a0013937ef)
...
- 完整的混合式遷移並不適合組織的所有類型。完整的混合式遷移的複雜性，因為具有小於一些好幾百種信箱組織看不到通常 justify 人力和一個設定所需的成本的優點。若此聲音您的組織，強烈建議您改用; 考慮轉換或基本混合移轉
...
> [!IMPORTANT]
> 如果您的組織選擇將信箱遷移至 Office 365，但要繼續進行管理使用者帳戶從內部部署 Active Directory 就地保留 DirSync 或 Azure AD 連線，您需要保留至少一個 Exchange server 的內部。如果移除前一個 Exchange server，您將不能夠變更 Exchange Online 中的 Exchange 收件者。這是因為授權來源會保留在內部部署 Active Directory 中且需要有進行變更。
...
當我們強烈相信您可以透過完全移轉至 Office 365 來達到最佳的值與使用者經驗時，我們也瞭解某些組織需要保留一些 Exchange 伺服器的內部。這可能是因為法規需求，以保證資料不儲存在位於另一個的國家/地區資料中心或其可能是因為您有唯一的設定] 或 [不能在雲端中符合需求，或只是可能是您需要的交換因為您仍然可以使用 Active Directory 內部管理雲端信箱。在任何情況下以您選擇或想要將 Exchange 內部部署，您應該確定 Exchange 2010 環境升級為至少 Exchange 2013 或 Exchange 2016 與 Exchange 2010 移除支援的結束日期之前。

最佳的體驗，我們建議您將剩餘的內部部署環境升級為 Exchange 2016。您不需要安裝 Exchange Server 2013 如果您想要提供 Exchange Server 2010 來自官方移至 Exchange Server 2016。

Exchange 2016 包含所有的功能和改進隨附於舊版的 Exchange 和其最符合適用 Office 365 的經驗 （雖然某些功能只適用於 Office 365）。查看只是一些您已被錯過的事項：

| **Exchange 版本**                     | **功能**                                                                                                                                                                                                                                         |
|------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Exchange 2013                            | 簡化的架構三降低伺服器角色的數字 (Mailbox、 Client Access Edge Transport)                                                                                                                                        |
|                                          | 資料外洩防護原則 (DLP) 協助保護機密資訊洩漏                                                                                                                                                                |
|                                          | 大幅改善的 Outlook Web App 功能                                                                                                                                                                                                    |
| Exchange 2016                            | *從 Exchange 2013 的功能和...]*                                                                                                                                                                                                                   |
|                                          | 進一步剛信箱及 Edge Transport 簡化的伺服器角色                                                                                                                                                                                   |
|                                          | 改善的 DLP 整合及與 SharePoint                                                                                                                                                                                                  |
|                                          | 改良的資料庫台回復性                                                                                                                                                                                                                         |
|                                          | 線上文件共同作業                                                                                                                                                                                                                        |

| **考量**                        | **更多資訊**                                                                                                                                                                                                                                        |
|------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 支援日期的結尾                     | 類似 Exchange 2010，每個版本的 Exchange 會有其專屬結尾支援日期：                                                                                                                                                                        |
|                                          | **Exchange 2013**年 4 月 2023                                                                                                                                                                                                                       |
|                                          | **Exchange 2016**年 10 月 2025                                                                                                                                                                                                                     |
|                                          | 稍早結尾的支援日期、 更快您需要執行其他移轉。4 月 2023年會比您想像的還很接近 ！                                                                                                                 |
| Exchange 2013 或 2016年的移轉路徑  | 不論您選擇 Exchange 2013 或 Exchange 2016 較新版從 Exchange 2010 的移轉路徑都一樣：                                                                                                                              |
|                                          | 安裝 Exchange 2013 或 2016年至現有的 Exchange 2010 組織移動服務與 Exchange 2013 或 2016年移動信箱的其他基礎結構與公用資料夾遷移至 Exchange 2013 或 2016年解除委任剩餘的 Exchange 2010 伺服器  |
| 版本共存                      | 移轉至 Exchange 2013 或 Exchange 2016 時，您可以安裝到現有的 Exchange 2010 組織的其中一個版本。這可讓您安裝一或多個 Exchange 2013 或 Exchange 2016 伺服器以及執行移轉。             |
| 伺服器硬體                          | 伺服器硬體需求已從 Exchange 2010 變更。您將需要確定您要使用的硬體相容。您可以找到更多有關以下每個版本的硬體需求：                                      |
...
|                                          | 您可以找到大幅改善 Exchange 效能與運算動力和較新的伺服器中的儲存容量，您可能需要較少的伺服器可支援相同的信箱數目。                       |
| 作業系統版本                 | 每個版本的最小支援的作業系統版本是：                                                                                                                                                                                |
...
| Active Directory 樹系功能等級 | 基本支援 Active Directory 樹系功能等級每個版本是：                                                                                                                                                                |
...
| Office 用戶端版本                   | 每個版本的最小支援的 Office 用戶端版本是：                                                                                                                                                                                   |
|                                          | **Exchange 2016**Office 2010 （含最新的更新）                                                                                                                                                                                              |
...
[可協助您升級從 Office 2010 伺服器和用戶端的資源](https://docs.microsoft.com/en-us/office365/enterprise/upgrade-from-office-2010-servers-and-products)

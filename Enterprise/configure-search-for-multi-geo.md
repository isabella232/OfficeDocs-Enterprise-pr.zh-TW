...
為多個地理位置環境已設定，使用者會看到新聞、 從在其 SharePoint 首頁] 頁面上的多個地理位置的最新和追蹤網站。如果他們使用搜尋方塊上的 SharePoint 首頁上，他們只能取得結果從搜尋索引位於的地理位置 SPHome。
...
<p>圖 1： 搜尋與三個地理位置的租用戶之間</p></th>
<th align="left"><p>搜尋用戶端會呼叫 EnableMultiGeoSearch 查詢屬性的搜尋 REST 端點 = true。</p>
<p>查詢會傳送到租用戶中的所有地理位置。</p>
<p>每個地理位置的搜尋結果會合併並排名。</p>
<p>用戶端取得整合的搜尋結果。</p></th>
</tr>
</thead>
<tbody>
...
3.  選取 [垂直若要設定，按一下 [**設定**齒輪圖示右上角，然後按一下 [**編輯頁面]**。

> ![](media/configure-search-for-multi-geo_image2.png)
>
> 在編輯模式中開啟的搜尋結果頁面。

1.  在搜尋結果網頁組件，將滑鼠指標移至左上右上角的 [網頁組件按一下箭頭，然後再按一下其功能表上的 [**編輯網頁組件**。 ![](media/configure-search-for-multi-geo_image3.png)

> 搜尋結果網頁組件工具窗格隨即開啟 [功能區中頂端右上方的頁面。
...
**EnableMultiGeoSearch** -這是一個布林值會指定是否查詢應扇形的其他地理位置的多個地理位置租用戶的索引。將它設為**true**可扇形查詢;**false**不扇形查詢。預設值為**false**。如果您未包含此參數，查詢是**不**扇形的其他地理位置。如果您不是多重地理位置環境中使用此參數，則會忽略此參數。
...
<tr class="odd">
<td align="left">無</td>
<td align="left">查詢無法在多個地理位置查詢，已不扇形的地理位置。</td>
</tr>
</tbody>
</table>

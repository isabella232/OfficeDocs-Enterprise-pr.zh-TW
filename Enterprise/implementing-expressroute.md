...
<span data-ttu-id="6bbbf-108">ExpressRoute for Office 365 可提供許多網際網路對向 Office 365 服務的備用路由路徑。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-108">ExpressRoute for Office 365 provides an alternate routing path to many internet facing Office 365 services.</span></span> <span data-ttu-id="6bbbf-109">Office 365 ExpressRoute 的架構是以 Office 365 服務（已透過網際網路存取） ExpressRoute 中的廣告公用 IP 首碼為基礎，以在您的網路中接下來再發佈這些 IP 首碼。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-109">The architecture of ExpressRoute for Office 365 is based on advertising public IP prefixes of Office 365 services that are already accessible over the Internet into your provisioned ExpressRoute circuits for subsequent redistribution of those IP prefixes into your network.</span></span> <span data-ttu-id="6bbbf-110">透過 ExpressRoute，您可以有效地為許多 Office 365 服務啟用多個不同的路由路徑（透過網際網路及透過 ExpressRoute）。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-110">With ExpressRoute you effectively enable several different routing paths, through the internet and through ExpressRoute, for many Office 365 services.</span></span> <span data-ttu-id="6bbbf-111">網路上路由的狀態可能代表如何設計內部網路拓撲的重大變更。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-111">This state of routing on your network may represent a significant change to how your internal network topology is designed.</span></span>
 ...
 <span data-ttu-id="6bbbf-112">**狀態：** 完成指南 v2</span><span class="sxs-lookup"><span data-stu-id="6bbbf-112">**Status:** Complete Guide v2</span></span>
...
4. <span data-ttu-id="6bbbf-125">您的小組已閱讀頻道9上的所有公開指導方針和檔， [https://aka.ms/expressrouteoffice365](https://aka.ms/expressrouteoffice365) [https://aka.ms/ert](https://aka.ms/ert) 並在頻道9上查看[Office 365 訓練系列的 Azure ExpressRoute](https://channel9.msdn.com/series/aer) ，以深入瞭解重要的技術詳細資料，包括：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-125">Your team has read all of the public guidance and documentation at [https://aka.ms/expressrouteoffice365](https://aka.ms/expressrouteoffice365), [https://aka.ms/ert](https://aka.ms/ert), and watched the [Azure ExpressRoute for Office 365 Training](https://channel9.msdn.com/series/aer) series on Channel 9 to gain an understanding of critical technical details including:</span></span>
      ...
      - <span data-ttu-id="6bbbf-126">SaaS 服務的網際網路相依性。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-126">The internet dependencies of SaaS services.</span></span>
...
- <span data-ttu-id="6bbbf-137">收集現有網路拓撲的檔，顯示內部 WAN 主幹和拓撲的詳細資料、衛星網站的連線、最後的使用者連線能力、路由傳送至網路周邊出局點，以及 proxy 服務。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-137">Gather documentation of existing network topology showing details of your internal WAN backbone and topology, connectivity of satellite sites, last mile user connectivity, routing to network perimeter egress points, and proxy services.</span></span>
  ...
  - <span data-ttu-id="6bbbf-140">識別所有 edge 裝置，例如 proxy、防火牆等等，以及將其關聯編目為透過網際網路和 ExpressRoute 的流量。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-140">Identify all edge devices, such as proxies, firewalls, and so on and catalog their relationship to flows going over the Internet and ExpressRoute.</span></span>
...
<span data-ttu-id="6bbbf-156">您的網路流量目錄應包含您的內部部署網路與 Microsoft 之間的所有輸入和輸出網路連線清單。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-156">Your network traffic catalog should contain listings of all the inbound and outbound network connections that you'll have between your on-premises network and Microsoft.</span></span>
...
|<span data-ttu-id="6bbbf-204">**Connection 屬性**</span><span class="sxs-lookup"><span data-stu-id="6bbbf-204">**Connection property**</span></span>|<span data-ttu-id="6bbbf-205">**值**</span><span class="sxs-lookup"><span data-stu-id="6bbbf-205">**Value**</span></span>|
|:-----|:-----|
...
1. <span data-ttu-id="6bbbf-236">將從其存取 Office 365 和其他服務的所有使用者位置。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-236">All user locations where Office 365 and other services will be accessed from.</span></span>
...
<span data-ttu-id="6bbbf-246">對於輸出流量，使用者會以下列三種方式之一存取 Office 365：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-246">For outbound traffic, the people access Office 365 in one of three ways:</span></span>
...
1. <span data-ttu-id="6bbbf-252">透過北美地區的 [會見-me] 位置來為加州人員提供人員。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-252">Through a meet-me location in North America for the people in California.</span></span>
...
- <span data-ttu-id="6bbbf-304">規劃頻寬、安全性、高可用性和容錯移轉。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-304">Plan for bandwidth, security, high availability and failover.</span></span>
...
#### <a name="plan-for-network-security-requirements"></a><span data-ttu-id="6bbbf-328">規劃網路安全性需求</span><span class="sxs-lookup"><span data-stu-id="6bbbf-328">Plan for network security requirements</span></span>
...
<span data-ttu-id="6bbbf-341">您可以用來符合上述需求的最常見方法，就是使用來源 NAT，以作為網路的一部分，或由您的 ExpressRoute 載體所提供。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-341">The most common method you can use to meet the above requirements is to use source NAT, either implemented as a part of your network or provided by your ExpressRoute carrier.</span></span> <span data-ttu-id="6bbbf-342">來源 NAT 可讓您從 ExpressRoute 摘要網際網路網路的詳細資料和私人 IP 位址，以及結合適當的 IP 路由通告，提供一種簡易的機制，以確保路徑對稱。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-342">Source NAT allows you to abstract the details and private IP addressing of your internet network from ExpressRoute and; coupled with proper IP route advertisements, provide an easy mechanism to ensure path symmetry.</span></span> <span data-ttu-id="6bbbf-343">如果您使用 ExpressRoute 對等位置特有的狀態網路裝置，則必須針對每個 ExpressRoute 對等位置執行個別的 NAT 集區，以確保路徑對稱。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-343">If you're using stateful network devices that are specific to ExpressRoute peering locations, you must implement separate NAT pools for each ExpressRoute peering to ensure path symmetry.</span></span>
...
<span data-ttu-id="6bbbf-353">您可以考慮下列其中一個執行模式，以滿足這項需求：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-353">You may consider one of the following implementation patterns to satisfy this requirement:</span></span>
...
1. <span data-ttu-id="6bbbf-383">當周邊網路安全時，傳入要求沒有來源 NAT 可用。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-383">While the perimeter network is secure, there is no Source NAT available for incoming requests.</span></span>
...
<span data-ttu-id="6bbbf-386">我們也會針對如何修正這些問題提出建議。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-386">We also have suggestions on how to fix them.</span></span>
...
##### <a name="solution-1a-source-nat"></a><span data-ttu-id="6bbbf-394">解決方案1a：來源 NAT</span><span class="sxs-lookup"><span data-stu-id="6bbbf-394">Solution 1a: Source NAT</span></span>
...
<span data-ttu-id="6bbbf-402">或者，您也可以選擇不允許公佈 ExpressRoute BGP 首碼，移除這些電腦的備用網路路徑。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-402">Alternatively, you can choose to not allow the ExpressRoute BGP prefixes to be advertised, removing the alternate network path for those computers.</span></span> <span data-ttu-id="6bbbf-403">在此圖表中：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-403">In this diagram:</span></span>
  
1. <span data-ttu-id="6bbbf-404">傳入要求會繼續透過新的 Jersey 資料中心周邊網路進入。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-404">The incoming request continues to enter through the New Jersey data center's perimeter network.</span></span> <span data-ttu-id="6bbbf-405">這段時間，新的 Jersey 資料中心無法使用透過 ExpressRoute 電路宣告給 Microsoft 的首碼。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-405">This time the prefixes advertised from Microsoft over the ExpressRoute circuit are not available to the New Jersey data center.</span></span>

2. <span data-ttu-id="6bbbf-406">伺服器發回的 IP 位址會傳回與原始 IP 位址相關聯的 IP 位址，而這會導致回應是沿著相同的網路路徑傳回。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-406">The response from the server routes back toward the IP associated with the original IP address over the only route available, resulting in the response returning along the same network path.</span></span>

![ExpressRoute Asymetric 路由解決方案2](media/9cb4b2bf-7aa6-487a-bc02-e02af8a979f6.png)
  
#### <a name="problem-2-cloud-to-on-premises-connection-over-expressroute"></a><span data-ttu-id="6bbbf-408">問題2： Cloud over ExpressRoute 上的內部部署連接</span><span class="sxs-lookup"><span data-stu-id="6bbbf-408">Problem 2: Cloud to on-premises connection over ExpressRoute</span></span>
  
<span data-ttu-id="6bbbf-409">下圖說明當您的網路設定不會提供來自 Microsoft cloud over ExpressRoute 之輸入要求的 NAT 時，所採用的非對稱網路路徑。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-409">The following diagram illustrates the asymmetric network path taken when your network configuration doesn't provide NAT for inbound requests from the Microsoft cloud over ExpressRoute.</span></span>
  
1. <span data-ttu-id="6bbbf-410">來自 Office 365 的輸入要求會從 DNS 中檢索 IP 位址，並將要求傳送至您的周邊網路。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-410">The inbound request from Office 365 retrieves the IP address from DNS and sends the request to your perimeter network.</span></span>

2. <span data-ttu-id="6bbbf-411">在此錯誤的設定中，未在傳送流量的周邊網路上設定或提供來源 NAT，進而產生實際來源 IP 位址，以當作傳回目的地使用。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-411">In this faulty configuration, there is no Source NAT configured or available at the perimeter network where the traffic is sent resulting in the actual source IP address being used as the return destination.</span></span>

  - <span data-ttu-id="6bbbf-412">網路上的電腦會透過任何可用的 ExpressRoute 網路連線，將傳回流量路由傳送至 Office 365。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-412">The computer on your network routes the return traffic to Office 365 through any available ExpressRoute network connection.</span></span>

  - <span data-ttu-id="6bbbf-413">其結果是與 Office 365 的非對稱連接。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-413">The result is an Asymmetric connection to Office 365.</span></span>

![ExpressRoute Asymetric 路由問題2](media/f6fd155b-bbb7-472a-846e-039a99f09913.png)
  
##### <a name="solution-2-source-nat"></a><span data-ttu-id="6bbbf-415">解決方案2：來源 NAT</span><span class="sxs-lookup"><span data-stu-id="6bbbf-415">Solution 2: Source NAT</span></span>
  
<span data-ttu-id="6bbbf-416">只要將來源 NAT 新增至輸入的要求，即可解析這種已設定錯誤的網路。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-416">Simply adding a source NAT to the inbound request resolves this misconfigured network.</span></span> <span data-ttu-id="6bbbf-417">在此圖表中：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-417">In this diagram:</span></span>
  
1. <span data-ttu-id="6bbbf-418">傳入的要求會繼續透過紐約資料中心的周邊網路進入。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-418">The incoming request continues to enter through the New York data center's perimeter network.</span></span> <span data-ttu-id="6bbbf-419">此時間源 NAT 可供使用。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-419">This time Source NAT is available.</span></span>

2. <span data-ttu-id="6bbbf-420">伺服器的回應會傳回與來源 NAT 相關聯的 IP 位址，而不是原始 IP 位址，進而導致回應沿著相同的網路路徑傳回。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-420">The response from the server routes back toward the IP associated with the Source NAT instead of the original IP address, resulting in the response returning along the same network path.</span></span>

![ExpressRoute Asymetric 路由解決方案3](media/a5d2b90d-a3ec-4047-afbf-6e6e99f376a7.png)
  
### <a name="paper-verify-that-the-network-design-has-path-symmetry"></a><span data-ttu-id="6bbbf-422">紙張驗證網路設計是否有對稱路徑</span><span class="sxs-lookup"><span data-stu-id="6bbbf-422">Paper verify that the network design has path symmetry</span></span>

<span data-ttu-id="6bbbf-423">此時，您必須驗證您的執行計畫是否為您將使用 Office 365 的不同案例提供路由對稱的功能。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-423">At this point, you need to verify on paper that your implementation plan offers route symmetry for the different scenarios in which you'll be using Office 365.</span></span> <span data-ttu-id="6bbbf-424">當人員使用服務的不同功能時，您會識別預計會採取的特定網路路由。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-424">You'll identify the specific network route that is expected to be taken when a person uses different features of the service.</span></span> <span data-ttu-id="6bbbf-425">從內部部署網路和 WAN 路由傳送至周邊裝置，到連接路徑;ExpressRoute 或網際網路，以及連線到線上端點的連線。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-425">From the on-premises network and WAN routing, to the perimeter devices, to the connectivity path; ExpressRoute or the internet, and on to the connection to the online endpoint.</span></span>
  
<span data-ttu-id="6bbbf-426">您必須針對先前識別為組織所採用之服務的所有 Office 365 網路服務執行這項作業。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-426">You'll need to do this for all of the Office 365 network services that were previously identified as services that your organization will adopt.</span></span>
  
<span data-ttu-id="6bbbf-427">這可協助您逐步完成與第二個人的路由。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-427">It helps to do this paper walk through of routes with a second person.</span></span> <span data-ttu-id="6bbbf-428">向使用者說明，每個網路躍點預期會從何處取得下一個路由，並確保您熟悉路由路徑。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-428">Explain to them where each network hop is expected to get its next route from and ensure that you're familiar with the routing paths.</span></span> <span data-ttu-id="6bbbf-429">請記住，ExpressRoute 一定會提供更具範圍的路由給 Microsoft server IP 位址，使其具有低於網際網路預設路由的路由成本。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-429">Remember that ExpressRoute will always provide a more scoped route to Microsoft server IP addresses giving it lower route cost than an Internet default route.</span></span>
  
### <a name="design-client-connectivity-configuration"></a><span data-ttu-id="6bbbf-430">設計 Client Connectivity Configuration</span><span class="sxs-lookup"><span data-stu-id="6bbbf-430">Design Client Connectivity Configuration</span></span>
<span data-ttu-id="6bbbf-431"><a name="asymmetric"> </a></span><span class="sxs-lookup"><span data-stu-id="6bbbf-431"><a name="asymmetric"> </a></span></span>

![使用具有 ExpressRoute 的 PAC 檔案](media/7cfa6482-dbae-416a-ae6f-a45e5f4de23b.png)
  
<span data-ttu-id="6bbbf-433">如果您使用 proxy 伺服器進行網際網路系結流量，則需要調整任何 PAC 或用戶端設定檔，以確保網路上的用戶端電腦已正確設定為將您想要的 ExpressRoute 流量傳送至 Office 365，而不需要 transiting proxy 伺服器，而其餘的流量（包括部分 Office 365 流量）會傳送至相關的 proxy。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-433">If you're using a proxy server for internet bound traffic then you need to adjust any PAC or client configuration files to ensure client computers on your network are correctly configured to send the ExpressRoute traffic you desire to Office 365 without transiting your proxy server, and the remaining traffic, including some Office 365 traffic, is sent to the relevant proxy.</span></span> <span data-ttu-id="6bbbf-434">如需[管理 Office 365 端點](https://aka.ms/manageo365endpoints)（例如 PAC 檔案）的指南，請參閱本指南。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-434">Read our guide on [managing Office 365 endpoints](https://aka.ms/manageo365endpoints) for example PAC files.</span></span>
  
> [!NOTE]
> <span data-ttu-id="6bbbf-435">端點經常變更，只要每週。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-435">The endpoints change frequently, as often as weekly.</span></span> <span data-ttu-id="6bbbf-436">您應根據貴組織已採用的服務和功能，只進行變更，以減少所需的變更數目，以減少保持最新的變更數目。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-436">You should only make changes based on the services and features your organization has adopted to reduce the number of changes you'll need to make to stay current.</span></span> <span data-ttu-id="6bbbf-437">請密切注意 RSS 摘要中的 [**有效日期**]：在此 rss 摘要中已宣告變更，且記錄保留所有過去的變更為止，已宣告的 IP 位址可能不會宣告或從通告中移除，直到達到有效日期為止。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-437">Pay close attention to the **Effective Date** in the RSS feed where the changes are announced and a record is kept of all past changes, IP addresses that are announced may not be advertised, or removed from advertisement, until the effective date is reached.</span></span>
  
## <a name="build-your-deployment-and-testing-procedures"></a><span data-ttu-id="6bbbf-438">建立部署及測試程式</span><span class="sxs-lookup"><span data-stu-id="6bbbf-438">Build your deployment and testing procedures</span></span>
<span data-ttu-id="6bbbf-439"><a name="testing"> </a></span><span class="sxs-lookup"><span data-stu-id="6bbbf-439"><a name="testing"> </a></span></span>

<span data-ttu-id="6bbbf-440">您的實施計畫應同時包含測試和回滾規劃。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-440">Your implementation plan should include both testing and rollback planning.</span></span> <span data-ttu-id="6bbbf-441">如果您的實施未如期運作，方案應設計為在探索問題之前，影響最少人數。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-441">If your implementation isn't functioning as expected, the plan should be designed to affect the least number of people before problems are discovered.</span></span> <span data-ttu-id="6bbbf-442">以下是您的方案應考慮的一些高層次原則。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-442">The following are some high level principles your plan should consider.</span></span>
  
1. <span data-ttu-id="6bbbf-443">階段網路段和使用者服務上架，以將中斷降至最低。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-443">Stage the network segment and user service onboarding to minimize disruption.</span></span>

2. <span data-ttu-id="6bbbf-444">規劃使用 traceroute 與 TCP 連線的測試路由，並從不同的網際網路連線主機進行連接。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-444">Plan for testing routes with traceroute and TCP connect from a separate internet connected host.</span></span>

3. <span data-ttu-id="6bbbf-445">在具有測試 Office 365 租使用者的隔離測試網路上，您最好會進行輸入和輸出服務的測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-445">Preferably, testing of inbound and outbound services should be done on an isolated test network with a test Office 365 tenant.</span></span>

      - <span data-ttu-id="6bbbf-446">或者，如果客戶尚未使用 Office 365 或進行試驗，也可以在生產網路上執行測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-446">Alternatively, testing can be performed on a production network if the customer is not yet using Office 365 or is in pilot.</span></span>

      - <span data-ttu-id="6bbbf-447">或者，您也可以在實際執行中斷期間進行測試，而不是僅供測試及監視之用。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-447">Alternatively, testing can be performed during a production outage that is set aside for test and monitoring only.</span></span>

      - <span data-ttu-id="6bbbf-448">或者，您可以在每個第三層路由器節點上檢查每個服務的路由，以進行測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-448">Alternatively, testing can be done by checking routes for each service on each layer 3 router node.</span></span> <span data-ttu-id="6bbbf-449">只有在缺乏實體測試帶來風險的情況時，才可使用此回退功能。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-449">This fall back should only be used if no other testing is possible since a lack of physical testing introduces risk.</span></span>

### <a name="build-your-deployment-procedures"></a><span data-ttu-id="6bbbf-450">建立部署程式</span><span class="sxs-lookup"><span data-stu-id="6bbbf-450">Build your deployment procedures</span></span>

<span data-ttu-id="6bbbf-451">您的部署程式應該向幾個階段中的小小組人員進行測試，然後再部署至較大的人員群組。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-451">Your deployment procedures should roll out to small groups of people in stages to allow for testing before deploying to larger groups of people.</span></span> <span data-ttu-id="6bbbf-452">以下是幾種方法來分段 ExpressRoute 的部署。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-452">The following are several ways to stage the deployment of ExpressRoute.</span></span>
  
1. <span data-ttu-id="6bbbf-453">使用 Microsoft 對等設定 ExpressRoute，並將路由播發轉送到單一主機，只用于分段測試目的。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-453">Set up ExpressRoute with Microsoft peering and have the route advertisements forwarded to a single host only for staged testing purposes.</span></span>

2. <span data-ttu-id="6bbbf-454">將路由傳送至 ExpressRoute 網路的單一網段，一開始，並依網路段或地區展開路由廣告。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-454">Advertise routes to the ExpressRoute network to a single network segment at first and expand route advertisements by network segment or region.</span></span>

3. <span data-ttu-id="6bbbf-455">如果是第一次部署 Office 365，請使用 ExpressRoute 網路部署做為少數人員的示範。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-455">If deploying Office 365 for the first time, use the ExpressRoute network deployment as a pilot for a small number of people.</span></span>

4. <span data-ttu-id="6bbbf-456">如果使用 proxy 伺服器，您也可以設定測試 PAC 檔案，以引導少量的人員在進行測試與意見 ExpressRoute 之前新增其他。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-456">If using proxy servers, you can alternatively configure a test PAC file to direct a small number of people to ExpressRoute with testing and feedback before adding more.</span></span>

<span data-ttu-id="6bbbf-457">您的實施計畫應該列出必須採取的每個部署程式，或需要用來部署網路設定的命令。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-457">Your implementation plan should list each of the deployment procedures that must be taken or commands that need to be used to deploy the networking configuration.</span></span> <span data-ttu-id="6bbbf-458">當網路停機時間到達所做的所有變更時，應從已預先撰寫的書面部署計畫和對等方檢查。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-458">When the network outage time arrives all of the changes being made should be from the written deployment plan which was written in advance and peer reviewed.</span></span> <span data-ttu-id="6bbbf-459">請參閱我們 ExpressRoute 之技術設定的指導方針。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-459">See our guidance on the technical configuration of ExpressRoute.</span></span>
  
- <span data-ttu-id="6bbbf-460">如果您已變更任何內部部署伺服器的 IP 位址以繼續傳送電子郵件，請更新您的 SPF TXT 記錄。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-460">Updating your SPF TXT records if you've changed IP addresses for any on-premises servers that will continue to send email.</span></span>

- <span data-ttu-id="6bbbf-461">如果您已變更 IP 位址以容納新的 NAT 設定，則為內部部署伺服器更新任何 DNS 專案。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-461">Updating any DNS entries for on-premises servers if you've changed IP addresses to accommodate a new NAT configuration.</span></span>

- <span data-ttu-id="6bbbf-462">確定您已訂閱 Office 365 端點通知的 RSS 摘要，以維護任何路由或 proxy 設定。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-462">Ensure you've subscribed to the RSS feed for Office 365 endpoint notifications to maintain any routing or proxy configurations.</span></span>

<span data-ttu-id="6bbbf-463">完成 ExpressRoute 部署後，應執行測試計劃中的程式。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-463">After your ExpressRoute deployment is complete the procedures in the test plan should be executed.</span></span> <span data-ttu-id="6bbbf-464">應該記錄每個程式的結果。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-464">Results for each procedure should be logged.</span></span> <span data-ttu-id="6bbbf-465">您必須在此事件中包含復原原始實際執行環境的程式，測試計劃結果表示執行失敗。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-465">You must include procedures for rolling back to the original production environment in the event the test plan results indicate the implementation was not successful.</span></span>
  
### <a name="build-your-test-procedures"></a><span data-ttu-id="6bbbf-466">建立測試程式</span><span class="sxs-lookup"><span data-stu-id="6bbbf-466">Build your test procedures</span></span>

<span data-ttu-id="6bbbf-467">您的測試程式應該包含每個用於 Office 365 的輸出和輸入網路服務的測試，將使用 ExpressRoute，而不是會使用。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-467">Your testing procedures should include tests for each outbound and inbound network service for Office 365 both that will be using ExpressRoute and ones that will not.</span></span> <span data-ttu-id="6bbbf-468">程式應包括從每個唯一的網路位置進行測試，包括在公司局域網中非內部部署的使用者。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-468">The procedures should include testing from each unique network location including users who are not on-premises in the corporate LAN.</span></span>
  
<span data-ttu-id="6bbbf-469">測試活動的一些範例包含下列各項。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-469">Some examples of test activities include the following.</span></span>
  
1. <span data-ttu-id="6bbbf-470">從您的內部部署路由器 Ping 至您的網路操作員路由器。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-470">Ping from your on-premises router to your network operator router.</span></span>

2. <span data-ttu-id="6bbbf-471">驗證您的內部部署路由器是否已接收 500 + Office 365 和 CRM Online IP 位址廣告。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-471">Validate the 500+ Office 365 and CRM Online IP address advertisements are received by your on-premises router.</span></span>

3. <span data-ttu-id="6bbbf-472">驗證您的輸入和輸出 NAT 是在 ExpressRoute 和內部網路之間運作。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-472">Validate your inbound and outbound NAT is operating between ExpressRoute and the internal network.</span></span>

4. <span data-ttu-id="6bbbf-473">驗證從您的路由器宣告到您的 NAT 的路由。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-473">Validate that routes to your NAT are being advertised from your router.</span></span>

5. <span data-ttu-id="6bbbf-474">驗證 ExpressRoute 已接受您宣告的首碼。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-474">Validate that ExpressRoute has accepted your advertised prefixes.</span></span>

      - <span data-ttu-id="6bbbf-475">使用下列 Cmdlet 來確認對等通告：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-475">Use the following cmdlet to verify peering advertisements:</span></span>

      ```PowerShell
      Get-AzureRmExpressRouteCircuitRouteTable -DevicePath Primary -ExpressRouteCircuitName TestER -ResourceGroupName RG -PeeringType MicrosoftPeering
      ```

6. <span data-ttu-id="6bbbf-476">驗證您的公用 NAT IP 範圍不會透過任何其他 ExpressRoute 或公用網際網路網路電路宣告給 Microsoft，除非它是先前範例中的較大範圍的特定子集。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-476">Validate your public NAT IP range is not advertised to Microsoft through any other ExpressRoute or public Internet network circuit unless it is a specific subset of a larger range as in the previous example.</span></span>

7. <span data-ttu-id="6bbbf-477">ExpressRoute 電路成對，請驗證兩個 BGP 會話是否都在執行中。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-477">ExpressRoute circuits are paired, validate that both BGP sessions are running.</span></span>

8. <span data-ttu-id="6bbbf-478">在 NAT 內設定單一主機，並使用 ping、tracert 和 tcpping 來測試跨新電路與主機 outlook.office365.com 的連線能力。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-478">Set up a single host on the inside of your NAT and use ping, tracert, and tcpping to test connectivity across the new circuit to the host outlook.office365.com.</span></span> <span data-ttu-id="6bbbf-479">或者，您也可以在鏡像的埠上使用 Wireshark 或 Microsoft 網路監視器3.4 之類的工具，以驗證您能夠連線至與 outlook.office365.com 相關聯的 IP 位址。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-479">Alternatively, you could use a tool such as Wireshark or Microsoft Network Monitor 3.4 on a mirrored port to the MSEE to validate you're able to connect to the IP address associated with outlook.office365.com.</span></span>

9. <span data-ttu-id="6bbbf-480">測試適用于 Exchange Online 的應用層級功能。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-480">Test application level functionality for Exchange Online.</span></span>

  - <span data-ttu-id="6bbbf-481">測試 Outlook 可連線至 Exchange Online 及傳送/接收電子郵件。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-481">Test Outlook is able to connect to Exchange Online and send/receive email.</span></span>

  - <span data-ttu-id="6bbbf-482">測試 Outlook 可使用線上模式。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-482">Test Outlook is able to use online-mode.</span></span>

  - <span data-ttu-id="6bbbf-483">測試 smartphone 連線能力及傳送/接收能力。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-483">Test smartphone connectivity and send/receive capability.</span></span>

10. <span data-ttu-id="6bbbf-484">測試 SharePoint 線上的應用層級功能</span><span class="sxs-lookup"><span data-stu-id="6bbbf-484">Test application level functionality for SharePoint Online</span></span>

  - <span data-ttu-id="6bbbf-485">測試 Business sync 用戶端的 OneDrive。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-485">Test OneDrive for Business sync client.</span></span>

  - <span data-ttu-id="6bbbf-486">測試 SharePoint 線上 web 存取。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-486">Test SharePoint Online web access.</span></span>

11. <span data-ttu-id="6bbbf-487">測試商務用 Skype 通話案例的應用層級功能：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-487">Test application level functionality for Skype for Business calling scenarios:</span></span>

  - <span data-ttu-id="6bbbf-488">加入電話會議做為已驗證的使用者 [由使用者初始化的邀請]。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-488">Join to conference call as authenticated user [invite initiated by end user].</span></span>

  - <span data-ttu-id="6bbbf-489">邀請使用者撥打電話會議 [從 MCU 傳送邀請]。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-489">Invite user to conference call [invite sent from MCU].</span></span>

  - <span data-ttu-id="6bbbf-490">使用商務用 Skype web 應用程式以匿名使用者的身分加入會議。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-490">Join conference as anonymous user using the Skype for Business web application.</span></span>

  - <span data-ttu-id="6bbbf-491">從您的有線電腦連線、IP 電話及行動裝置加入通話。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-491">Join call from your wired PC connection, IP phone, and mobile device.</span></span>

  - <span data-ttu-id="6bbbf-492">呼叫同盟使用者 o 呼叫至 PSTN 驗證：通話已完成，通話品質可接受，連線時間是可接受的。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-492">Call to federated user o Call to PSTN Validation: call is completed, call quality is acceptable, connection time is acceptable.</span></span>

  - <span data-ttu-id="6bbbf-493">確認已針對承租人和同盟使用者的兩個成員更新連絡人的狀態。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-493">Verify presence status for contacts is updated for both members of the tenant and federated users.</span></span>

### <a name="common-problems"></a><span data-ttu-id="6bbbf-494">常見問題</span><span class="sxs-lookup"><span data-stu-id="6bbbf-494">Common problems</span></span>

<span data-ttu-id="6bbbf-495">非對稱路由是最常見的實施問題。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-495">Asymmetric routing is the most common implementation problem.</span></span> <span data-ttu-id="6bbbf-496">以下是一些要尋找的常見來源：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-496">Here are some common sources to look for:</span></span>
  
- <span data-ttu-id="6bbbf-497">就地使用開放或單層網路路由拓撲，但沒有源 NAT。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-497">Using an open or flat network routing topology without source NAT in place.</span></span>

- <span data-ttu-id="6bbbf-498">不使用 SNAT 透過網際網路和 ExpressRoute 連接來路由傳送至輸入服務。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-498">Not using SNAT to route to inbound services through both the internet and ExpressRoute connections.</span></span>

- <span data-ttu-id="6bbbf-499">在部署廣泛之前，請勿在測試網路上測試 ExpressRoute 上的輸入服務。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-499">Not testing inbound services on ExpressRoute on a test network prior to deploying broadly.</span></span>

## <a name="deploying-expressroute-connectivity-through-your-network"></a><span data-ttu-id="6bbbf-500">透過您的網路部署 ExpressRoute 連通性</span><span class="sxs-lookup"><span data-stu-id="6bbbf-500">Deploying ExpressRoute connectivity through your network</span></span>
<span data-ttu-id="6bbbf-501"><a name="testing"> </a></span><span class="sxs-lookup"><span data-stu-id="6bbbf-501"><a name="testing"> </a></span></span>

<span data-ttu-id="6bbbf-502">將您的部署逐步劃分到網路的一個網段，以逐步向網路的不同部分推出，並以每個新的網路區段復原的計畫。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-502">Stage your deployment to one segment of the network at a time, progressively rolling out the connectivity to different parts of the network with a plan to roll back for each new network segment.</span></span> <span data-ttu-id="6bbbf-503">如果您的部署與 Office 365 部署一致，請先部署 Office 365 試驗使用者，然後再從那裡進行擴充。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-503">If your deployment is aligned with an Office 365 deployment, deploy to your Office 365 pilot users first and extend from there.</span></span>
  
<span data-ttu-id="6bbbf-504">先進行測試，然後再進行實際執行：</span><span class="sxs-lookup"><span data-stu-id="6bbbf-504">First for your test and then for production:</span></span>
  
- <span data-ttu-id="6bbbf-505">執行部署步驟以啟用 ExpressRoute。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-505">Run the deployment steps to enable ExpressRoute.</span></span>

- <span data-ttu-id="6bbbf-506">測試您的查看網路路由是否如預期。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-506">Test your seeing the network routes are as expected.</span></span>

- <span data-ttu-id="6bbbf-507">在每個輸入和輸出服務上執行測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-507">Perform testing on each inbound and outbound service.</span></span>

- <span data-ttu-id="6bbbf-508">如果您探索任何問題，就會復原。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-508">Rollback if you discover any issues.</span></span>

### <a name="set-up-a-test-connection-to-expressroute-with-a-test-network-segment"></a><span data-ttu-id="6bbbf-509">使用測試網路段設定 ExpressRoute 的測試連接</span><span class="sxs-lookup"><span data-stu-id="6bbbf-509">Set up a test connection to ExpressRoute with a test network segment</span></span>

<span data-ttu-id="6bbbf-510">現在，您已完成的計畫是在一小段時間進行測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-510">Now that you have the completed plan on paper it is time to test at a small scale.</span></span> <span data-ttu-id="6bbbf-511">在此測試中，您將建立單一 ExpressRoute 連線與 Microsoft 對等網路對內部部署網路上的測試子網的關聯。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-511">In this test you will establish a single ExpressRoute connection with Microsoft Peering to a test subnet on your on-premises network.</span></span> <span data-ttu-id="6bbbf-512">您可以設定[試用版 Office 365 租](https://go.microsoft.com/fwlink/p/?LinkID=403802)使用者與 test 子網間的連線，並在測試子網中包含您將在生產環境中使用的所有輸出和輸入服務。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-512">You can configure a [trial Office 365 tenant](https://go.microsoft.com/fwlink/p/?LinkID=403802) with connectivity to and from the test subnet and include all outbound and inbound services that you will be using in production in the test subnet.</span></span> <span data-ttu-id="6bbbf-513">設定測試網路段的 DNS，並建立所有輸入和輸出服務。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-513">Set up DNS for the test network segment and establish all inbound and outbound services.</span></span> <span data-ttu-id="6bbbf-514">執行測試計劃，並確定您熟悉每個服務和路由傳播的路由。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-514">Execute your test plan and ensure that you are familiar with the routing for each service and the route propagation.</span></span>
  
### <a name="execute-the-deployment-and-test-plans"></a><span data-ttu-id="6bbbf-515">執行部署和測試計劃</span><span class="sxs-lookup"><span data-stu-id="6bbbf-515">Execute the deployment and test plans</span></span>

<span data-ttu-id="6bbbf-516">當您完成上述專案時，請核對您已完成的區域，並確定您和您的小組已在執行部署和測試計劃之前，先查看這些專案。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-516">As you complete the items described above, check off the areas you've completed and ensure you and your team have reviewed them before executing your deployment and testing plans.</span></span>
  
- <span data-ttu-id="6bbbf-517">網路變更所涉及的輸出和輸入服務清單。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-517">List of outbound and inbound services that are involved in the network change.</span></span>

- <span data-ttu-id="6bbbf-518">全域網路架構圖表顯示網際網路出口和 ExpressRoute 的 [符合我的位置] 位置。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-518">Global network architecture diagram showing both internet egress and ExpressRoute meet-me locations.</span></span>

- <span data-ttu-id="6bbbf-519">示範用於每個已部署服務之不同網路路徑的網路路由圖表。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-519">Network routing diagram demonstrating the different network paths used for each service deployed.</span></span>

- <span data-ttu-id="6bbbf-520">部署計畫，包含必要時執行變更和回滾的步驟。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-520">A deployment plan with steps to implement the changes and rollback if needed.</span></span>

- <span data-ttu-id="6bbbf-521">測試每個 Office 365 和網路服務的測試計劃。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-521">A test plan for testing each Office 365 and network service.</span></span>

- <span data-ttu-id="6bbbf-522">已完成輸入和輸出服務之實際執行路由的紙張驗證。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-522">Completed paper validation of production routes for inbound and outbound services.</span></span>

- <span data-ttu-id="6bbbf-523">測試網段上的完整測試，包括可用性測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-523">A completed test across a test network segment including availability testing.</span></span>

<span data-ttu-id="6bbbf-524">選擇一個長時間足以透過整個部署計畫及測試計劃執行的中斷時段，並有一些時間可供進行疑難排解，以及在必要時回復的時間。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-524">Choose an outage window that is long enough to run through the entire deployment plan and the test plan, has some time available for troubleshooting and time for rolling back if required.</span></span>
  
> [!CAUTION]
> <span data-ttu-id="6bbbf-525">由於透過網際網路和 ExpressRoute 進行路由的複雜性質，建議您在此視窗中新增額外的緩衝時間，以處理複雜路由疑難排解。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-525">Due to the complex nature of routing over both the internet and ExpressRoute, it is recommended that additional buffer time is added to this window to handle troubleshooting complex routing.</span></span>
  
### <a name="configure-qos-for-skype-for-business-online"></a><span data-ttu-id="6bbbf-526">為商務用 Skype Online 設定 QoS</span><span class="sxs-lookup"><span data-stu-id="6bbbf-526">Configure QoS for Skype for Business Online</span></span>

<span data-ttu-id="6bbbf-527">QoS 是取得商務用 Skype Online 之語音和會議效益的必要。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-527">QoS is necessary to obtain voice and meeting benefits for Skype for Business Online.</span></span> <span data-ttu-id="6bbbf-528">您可以在確保 ExpressRoute 網路連接不會封鎖任何其他 Office 365 服務存取之後，設定 QoS。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-528">You can configure QoS after you have ensured that the ExpressRoute network connection does not block any of your other Office 365 service access.</span></span> <span data-ttu-id="6bbbf-529">QoS 的設定會在[ExpressRoute 和 QoS 的商務用 Skype Online](https://support.office.com/article/ExpressRoute-and-QoS-in-Skype-for-Business-Online-20c654da-30ee-4e4f-a764-8b7d8844431d)中說明。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-529">Configuration for QoS is described in the article [ExpressRoute and QoS in Skype for Business Online](https://support.office.com/article/ExpressRoute-and-QoS-in-Skype-for-Business-Online-20c654da-30ee-4e4f-a764-8b7d8844431d) .</span></span>
  
## <a name="troubleshooting-your-implementation"></a><span data-ttu-id="6bbbf-530">您的實施疑難排解</span><span class="sxs-lookup"><span data-stu-id="6bbbf-530">Troubleshooting your implementation</span></span>
<span data-ttu-id="6bbbf-531"><a name="troubleshooting"> </a></span><span class="sxs-lookup"><span data-stu-id="6bbbf-531"><a name="troubleshooting"> </a></span></span>

<span data-ttu-id="6bbbf-532">第一個要注意的地方是在實施方案中未錯過任何專案，在此實施方案中的步驟。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-532">The first place to look is at the steps in this implementation guide, were any missed in your implementation plan?</span></span> <span data-ttu-id="6bbbf-533">若有可能複製錯誤並加以調試，請傳回並執行進一步的小型網路測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-533">Go back and run further small network testing if possible to replicate the error and debug it there.</span></span>
  
<span data-ttu-id="6bbbf-534">識別測試期間哪些輸入或輸出服務失敗。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-534">Identify which inbound or outbound services failed during testing.</span></span> <span data-ttu-id="6bbbf-535">針對每個失敗的服務，明確取得 IP 位址與子網。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-535">Get specifically the IP addresses and subnets for each of the services which failed.</span></span> <span data-ttu-id="6bbbf-536">繼續進行並流覽紙張上的網路拓撲圖表，並驗證路由。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-536">Go ahead and walk the network topology diagram on paper and validate the routing.</span></span> <span data-ttu-id="6bbbf-537">特別驗證通告 ExpressRoute 路由的位置，在中斷期間（如果可能有追蹤）進行路由測試。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-537">Validate specifically where the ExpressRoute routing is advertised to, Test that routing during the outage if possible with traces.</span></span>
  
<span data-ttu-id="6bbbf-538">以網路追蹤方式執行 PSPing 每個用戶端點，並評估來源和目的地 IP 位址，以驗證其是否如預期。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-538">Run PSPing with a network trace to each customer endpoint and evaluate source and destination IP addresses to validate that they are as expected.</span></span> <span data-ttu-id="6bbbf-539">對您在埠25公開的任何郵件主機執行 telnet，確認在預期的情況中，SNAT 會隱藏原始來源 IP 位址。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-539">Run telnet to any mail host that you expose on port 25 and verify that SNAT is hiding the original source IP address if this is expected.</span></span>
  
<span data-ttu-id="6bbbf-540">請記住，當您使用 ExpressRoute 連線部署 Office 365 時，您必須確定 ExpressRoute 的網路設定為最優化設計，而且您也對網路上的其他元件（例如用戶端電腦）進行優化。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-540">Keep in mind that while deploying Office 365 with an ExpressRoute connection you'll need to ensure both the network configuration for ExpressRoute is optimally designed and you've also optimized the other components on your network such as client computers.</span></span> <span data-ttu-id="6bbbf-541">除了使用此規劃指南來疑難排解可能錯過的步驟之外，我們也撰寫了[Office 365 的性能疑難排解計畫](https://support.office.com/article/Performance-troubleshooting-plan-for-Office-365-e241e5d9-b1d8-4f1d-a5c8-4106b7325f8c)。</span><span class="sxs-lookup"><span data-stu-id="6bbbf-541">In addition to using this planning guide to troubleshoot the steps you may have missed, we also have written a [Performance troubleshooting plan for Office 365](https://support.office.com/article/Performance-troubleshooting-plan-for-Office-365-e241e5d9-b1d8-4f1d-a5c8-4106b7325f8c) .</span></span>
  
<span data-ttu-id="6bbbf-542">您可以使用下列短連結返回這裡：[https://aka.ms/implementexpressroute365](https://aka.ms/implementexpressroute365)</span><span class="sxs-lookup"><span data-stu-id="6bbbf-542">Here's a short link you can use to come back: [https://aka.ms/implementexpressroute365](https://aka.ms/implementexpressroute365)</span></span>
  
## <a name="related-topics"></a><span data-ttu-id="6bbbf-543">相關主題</span><span class="sxs-lookup"><span data-stu-id="6bbbf-543">Related Topics</span></span>

[<span data-ttu-id="6bbbf-544">評估 Office 365 的網路連線能力</span><span class="sxs-lookup"><span data-stu-id="6bbbf-544">Assessing Office 365 network connectivity</span></span>](assessing-network-connectivity.md)
  
[<span data-ttu-id="6bbbf-545">Azure ExpressRoute for Office 365</span><span class="sxs-lookup"><span data-stu-id="6bbbf-545">Azure ExpressRoute for Office 365</span></span>](azure-expressroute.md)
  
[<span data-ttu-id="6bbbf-546">管理 ExpressRoute for Office 365 連線</span><span class="sxs-lookup"><span data-stu-id="6bbbf-546">Managing ExpressRoute for Office 365 connectivity</span></span>](managing-expressroute-for-connectivity.md)
  
[<span data-ttu-id="6bbbf-547">使用 ExpressRoute for Office 365 進行路由傳送</span><span class="sxs-lookup"><span data-stu-id="6bbbf-547">Routing with ExpressRoute for Office 365</span></span>](routing-with-expressroute.md)
  
[<span data-ttu-id="6bbbf-548">使用 ExpressRoute for Office 365 進行網路規劃</span><span class="sxs-lookup"><span data-stu-id="6bbbf-548">Network planning with ExpressRoute for Office 365</span></span>](network-planning-with-expressroute.md)
  
[<span data-ttu-id="6bbbf-549">在適用于 Office 365 案例的 ExpressRoute 中使用 BGP 社區（預覽）</span><span class="sxs-lookup"><span data-stu-id="6bbbf-549">Using BGP communities in ExpressRoute for Office 365 scenarios (preview)</span></span>](bgp-communities-in-expressroute.md)
  
[<span data-ttu-id="6bbbf-550">商務用 Skype Online 中的媒體品質和網路連線效能</span><span class="sxs-lookup"><span data-stu-id="6bbbf-550">Media Quality and Network Connectivity Performance in Skype for Business Online</span></span>](https://support.office.com/article/5fe3e01b-34cf-44e0-b897-b0b2a83f0917)
  
[<span data-ttu-id="6bbbf-551">針對商務用 Skype Online 最佳化您的網路</span><span class="sxs-lookup"><span data-stu-id="6bbbf-551">Optimizing your network for Skype for Business Online</span></span>](https://support.office.com/article/b363bdca-b00d-4150-96c3-ec7eab5a8a43)
  
[<span data-ttu-id="6bbbf-552">商務用 Skype Online 中的 ExpressRoute 與 QoS</span><span class="sxs-lookup"><span data-stu-id="6bbbf-552">ExpressRoute and QoS in Skype for Business Online</span></span>](https://support.office.com/article/20c654da-30ee-4e4f-a764-8b7d8844431d)
  
[<span data-ttu-id="6bbbf-553">使用 ExpressRoute 的通話流程</span><span class="sxs-lookup"><span data-stu-id="6bbbf-553">Call flow using ExpressRoute</span></span>](https://support.office.com/article/413acb29-ad83-4393-9402-51d88e7561ab)
  
[<span data-ttu-id="6bbbf-554">使用基準與效能歷程記錄進行 Office 365 效能調整</span><span class="sxs-lookup"><span data-stu-id="6bbbf-554">Office 365 performance tuning using baselines and performance history</span></span>](performance-tuning-using-baselines-and-history.md)
  
[<span data-ttu-id="6bbbf-555">Office 365 的效能疑難排解規劃</span><span class="sxs-lookup"><span data-stu-id="6bbbf-555">Performance troubleshooting plan for Office 365</span></span>](performance-troubleshooting-plan.md)
  
[<span data-ttu-id="6bbbf-556">Office 365 URL 與 IP 位址範圍</span><span class="sxs-lookup"><span data-stu-id="6bbbf-556">Office 365 URLs and IP address ranges</span></span>](https://support.office.com/article/8548a211-3fe7-47cb-abb1-355ea5aa88a2)
  
[<span data-ttu-id="6bbbf-557">Office 365 網路與效能調整</span><span class="sxs-lookup"><span data-stu-id="6bbbf-557">Office 365 network and performance tuning</span></span>](network-planning-and-performance.md)

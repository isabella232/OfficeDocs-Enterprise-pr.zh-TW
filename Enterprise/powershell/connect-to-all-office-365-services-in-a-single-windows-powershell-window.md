...
---
# <a name="connect-to-all-microsoft-365-services-in-a-single-windows-powershell-window"></a>連接到單一 Windows PowerShell 視窗中的所有 Microsoft 365 服務

當您使用 PowerShell 來管理 Microsoft 365 時，最多可以同時開啟五個不同的 Windows PowerShell 會話，其對應于 Microsoft 365 系統管理中心、SharePoint 線上、Exchange Online、商務用 Skype Online、Microsoft 團隊及安全性與 &amp; 合規性中心。 您的桌面可以在不同的 Windows PowerShell 會話中使用五種不同的連接方法，如下所示：
  
![一次執行五個 Windows PowerShell 主控台](media/a1a852c2-89ea-4e8e-8d8b-dcdf596763d1.png)
  
這對管理 Microsoft 365 並不是最佳的，因為您無法在這五個 windows 間交換資料，以進行跨服務管理。 本主題說明如何使用單一 Windows PowerShell 實例，您可以在其中管理 Microsoft 365 帳戶、商務用 Skype Online、Exchange Online、SharePoint 線上、Microsoft 團隊及安全性與 &amp; 合規性中心。

>[!Note]
>本文目前只包含連接到全球（+ GCC）雲端的命令。 其他附注提供有關連接至其他 Microsoft 365 雲端資訊的文章連結。
>

## <a name="before-you-begin"></a>開始之前

在您可以從單一 Windows PowerShell 實例管理所有 Microsoft 365 之前，請考慮下列必要條件：
  
- 您用來執行這些程式的 Microsoft 365 工作或學校帳戶，必須是 Microsoft 365 系統管理員角色的成員。 如需詳細資訊，請參閱[系統管理員角色](https://docs.microsoft.com/microsoft-365/admin/add-users/about-admin-roles?view=o365-worldwide)。 這是 Microsoft 365 PowerShell 的需求，不一定是所有其他 Microsoft 365 服務。
...
- 您可以使用下列 Windows 64 位元版本：
  ...
  - Windows Server 2008 R2 SP1*
    ...
    您必須使用64位版本的 Windows，因為商務用 Skype Online 模組和其中一個 Microsoft 365 模組的需求。
...
## <a name="connection-steps-when-using-just-a-password"></a>僅使用密碼時的連接步驟

以下是當您使用登入密碼時，連線至單一 PowerShell 視窗中所有服務的步驟。
  
1. 開啟 [Windows PowerShell]。
    
2. 執行這個命令，並輸入您的 Microsoft 365 工作或學校帳號憑證。
...
4. 執行這些命令，以連線至 SharePoint 線上。 指定您網域的組織名稱。 例如，若為 "litwareinc.onmicrosoft.com"，則組織名稱值為 "litwareinc"。
...
>[!Note]
>若要連線至全球以外的 Microsoft 365 雲端的 Exchange Online，請使用 **-ExchangeEnvironmentName**參數。 如需詳細資訊，請參閱[Connect-ExchangeOnline](https://docs.microsoft.com/powershell/module/exchange/powershell-v2-module/connect-exchangeonline?view=exchange-ps) 。
>
...
>[!Note]
>若要連接至全球以外的 Microsoft 365 雲端的安全性與 &amp; 合規性中心，請參閱[Connect to Security & 合規性中心 PowerShell](https://docs.microsoft.com/powershell/exchange/office-365-scc/connect-to-scc-powershell/connect-to-scc-powershell)。
>
...
如需安全 &amp; 規範中心，請參閱[Connect to Security & 合規性中心 PowerShell 使用多重要素驗證](https://docs.microsoft.com/powershell/exchange/office-365-scc/connect-to-scc-powershell/mfa-connect-to-scc-powershell?view=exchange-ps)連線以使用多重要素驗證：
